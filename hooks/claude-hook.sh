...
Open loops: memory/open-loops.md tracks ${olc} item(s) — surface any that look stale to the user."; added=1; }; fi
  ...
  post-tool-use)
    ...
    case "$rel" in
      memory/audits/*.md) if [ -f "$f" ]; then pr="${CLAUDE_PLUGIN_ROOT:-$(cd "$(dirname "$0")/.." 2>/dev/null && pwd -P)}"; av="$pr/scripts/validate-audit-artifact.py"; [ -f "$av" ] || block "Artifact Gate failure in $rel: validator unavailable at $av."; ve="$(python3 "$av" "$f" --relative-path "$rel" 2>&1)" || block "Artifact Gate failure in $rel. The v3 audit contract is fail-closed: $ve"; fi;;
