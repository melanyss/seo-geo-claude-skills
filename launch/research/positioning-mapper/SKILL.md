...
1. **Confirm the product, stage, and launch scope** — what is being positioned, and at what stage (draft / alpha / beta / GA). Read the stage record from `memory/launch-registry/` when present; if you surface a new stage/date fact, submit it to `memory/events/launches.ndjson` via an authorized `operation: propose` request to `registry-events.py` rather than asserting it. Positioning a GA narrative for a beta product is the upstream of a later `R1` stage-truth failure.
