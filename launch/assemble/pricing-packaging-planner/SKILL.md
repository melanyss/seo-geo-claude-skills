...
5. **Plan the beta / early-adopter price path** — the beta or early-adopter price, whether early adopters keep their rate after GA (grandfathering), and the declared post-GA price. Pricing state per stage feeds `R1` stage-truth (a GA announcement needs a live pricing page): submit stage-linked pricing facts to `memory/events/launches.ndjson` via an authorized `operation: propose` request to `registry-events.py`; [launch-registry](../../../protocol/launch-registry/SKILL.md) is the SSOT for stage and date.
