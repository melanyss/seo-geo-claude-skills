...
`email-quality-auditor` (SEND), `launch-readiness-auditor` (RAMP), and `social-quality-auditor` (ECHO)
...
→ `memory/audits/launch/`, `social-quality-auditor` → `memory/audits/social/`). content-reviewer maps its C³ ART verdict to
