...
- **Source digest:** `sha256:f0276696e347879bd1c759af2ae5319e5baff146cf096f0dd1143549c8f6c63f`
...
## Embedded Source: `references/c3-benchmark.md`
...
This file is the human entry point. Executable identity, profiles, evidence policy, missingness, vetoes, and rollup constraints live in `framework-catalog.json`; shared semantics live in `scoring-semantics.md`. The detailed item anchors live under `references/c3/`.
...
Exactly one verified veto failure caps that scope at `min(raw, 59)`. Two or more verified veto failures produce `verdict: BLOCK` with no final score. Missing veto evidence is `unknown`, not a failure. See `scoring-semantics.md` for status/verdict behavior.
...
- `c3/scoring-architecture.md` — item scoring, profiles, boundaries, and rollup
- `c3/ace-creator-benchmark.md` — Creator/ACE anchors
- `c3/art-content-benchmark.md` — Content/ART anchors
- `c3/roi-campaign-benchmark.md` — Campaign/ROI anchors

## Skill Ownership

- **Discover** — `fit-scorer` produces ACE; `influencer-discovery` only supplies candidates/evidence.
- **Activate** — `content-reviewer` produces ART and applies `ART.T1/T2`.
- **Measure** — `roi-calculator` produces ROI and CVI; `performance-analyzer` supplies measured inputs.

All outputs remain advisory until the versioned profile passes the reliability and outcome-calibration protocol in `scoring-semantics.md`.

## Embedded Source: `references/c3/art-content-benchmark.md`

# ART — Content Benchmark

> Scores a single **content deliverable** (post / video / reel / story) — *"is this content good & compliant?"*
> Tier-2 rubric of the C³ Scoring System. Scores a single deliverable on its own merits — craft, fit, and honesty.
...
**Boundary rule:** ART scores whether the content is *good and legal* — **not whether it converted** (that is measured in ROI.Impact). CTA **presence** is scored here (R4); CTA **performance** is ROI's.
...
## Embedded Source: `references/c3/scoring-architecture.md`
...
A scope score requires 100% coverage of applicable items. Missing or refused evidence is `unknown`, not Partial/Fail; catalog-authorized inapplicability is `na` with reason. One verified veto caps at 59; 2+ verified vetoes produce `BLOCK` with no final score. See `scoring-semantics.md`.
...
Use the `components` input in `c3-rollup.schema.json` for multi-creator/multi-asset campaigns. The deterministic runtime requires a positive budget weight for every ACE component when more than one creator is present, applies equal weight to ART, and requires exactly one ROI component.
...
- **ACE** — Creator benchmark → `ace-creator-benchmark.md`
- **ART** — Content benchmark → `art-content-benchmark.md`
- **ROI** — Campaign benchmark → `roi-campaign-benchmark.md`

## Embedded Source: `references/c3-rollup.schema.json`

```json
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "$id": "https://github.com/aaron-he-zhu/aaron-marketing-skills/references/c3-rollup.schema.json",
  "title": "C3 Campaign Value Index Rollup Input",
  "type": "object",
    ...
    },
    {
      ...
      "properties": {
        "components": {
          "type": "object",
          "additionalProperties": false,
          "required": ["ace", "art", "roi"],
          "properties": {
            "ace": {
              "type": "array",
              "minItems": 1,
              "items": {"$ref": "#/$defs/aceComponent"}
            },
            "art": {
              "type": "array",
              "minItems": 1,
              "items": {"$ref": "#/$defs/equalComponent"}
            },
            "roi": {
              "type": "array",
              "minItems": 1,
  ...
  "$defs": {
    "scoredResult": {
      "type": "object",
      "required": [
        "framework", "catalog_version", "profile", "target", "observed_at",
        "context", "score_state", "final_overall_score"
      ],
      "properties": {
        "framework": {"const": "C3"},
        "catalog_version": {"type": "string", "minLength": 1},
        "profile": {"pattern": "^(ace|art|roi)-(awareness|engagement|conversion|brand-building)$"},
        "target": {"type": "string", "minLength": 1},
        "observed_at": {"type": "string", "format": "date"},
        "context": {
          "type": "object",
          "required": ["scope", "goal", "assessment_time", "rollup_id"]
        },
    ...
    "equalComponent": {
      ...
      "additionalProperties": false,
      "required": ["result"],
      "properties": {"result": {"$ref": "#/$defs/scoredResult"}}
    }
  }
}
```
