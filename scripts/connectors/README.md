...
| `youtube.py` | YouTube Data API v3 creator metrics: real subscriber/view/video counts (`channel`) + per-video views/likes/comments (`videos`) — **shortlist vetting + own-campaign measurement, not bulk harvesting (ToS)** | `youtube.py channel @handle` · `youtube.py videos @handle --limit 10` | YouTube Data API (free key, `YOUTUBE_API_KEY`, 10k units/day) |
