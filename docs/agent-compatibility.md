...
bash scripts/publish-clawhub.sh --dry-run  # preview all 86 (verified: 86/86 resolve)
...
bash scripts/publish-skillhub.sh --dry-run     # local pre-check, all 86 (verified: 86/86 pass)
...
|-------|-------------|------------|-------|
| **Claude Code** | `.claude/skills/` (+ plugin skills) | `~/.claude/skills/` | Prefer the plugin — it adds the 6 commands, hooks, memory, connectors. Does **not** read `.agents/skills/`; `npx skills` handles the mapping. |
...
|-----------------|---------------------|
| Repo-root `references/` (auditor runbook, CORE-EEAT / CITE / C³ / ROAS / SEND / RAMP benchmarks, skill contract, state model) | Relative links break. The 6 auditor gates carry an explicit fallback: fetch the file from `https://raw.githubusercontent.com/aaron-he-zhu/aaron-marketing-skills/main/references/<filename>` or ask for a repo clone — they never score without the runbook. Non-gate skills inline their essential rules, so broken links are cosmetic. |
...
| The 6 `/aaron-marketing:*` commands | Claude Code plugin only. On other hosts, describe the goal — skill descriptions carry the routing triggers. |
...
**Positioning in one line**: Claude Code plugin = the operated product (gates enforced, memory persisted, connectors wired); any other host = the same 86 skill procedures, self-contained.
