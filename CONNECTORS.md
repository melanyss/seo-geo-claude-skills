...
| Social posts search, keyless (`~~social listening`) | [Bluesky public AppView](https://docs.bsky.app/docs/api/app-bsky-feed-search-posts) | `GET https://public.api.bsky.app/xrpc/app.bsky.feed.searchPosts?q=…` | none for basic search (cursor pagination requires auth; datacenter IPs may be throttled) | be gentle |
| Hashtag/instance timelines, keyless (`~~social listening`) | [Mastodon public API](https://docs.joinmastodon.org/methods/timelines/) | `GET https://<instance>/api/v1/timelines/tag/<tag>` — per-instance public data | none on most instances | per-instance limits |
| Community discussion search (`~~social listening`, `~~trend database`) | [Reddit API](https://www.reddit.com/dev/api/) | search/subreddit listings on the free tier | free OAuth app (~100 queries/min) | free tier |
...
| Social Listening | `~~social listening` | both | Brandwatch, Sprout Social, Talkwalker | Google Alerts / F5Bot / Bluesky public search (keyless) / Mastodon public timelines (keyless) / Reddit free OAuth / self-host RSSHub | Google Alerts RSS + Bluesky keyless search |
